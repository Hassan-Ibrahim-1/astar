...
std::optional<std::vector<Cell *>> Path::trace(Cell *start_cell,
                                               Cell *target_cell, Grid &grid) {
  ...
  start_node.g = 0;
  start_node.h =
      calculate_distance_to_target(grid.cells[start_node.cell_index]);
...
float Path::calculate_distance_from_start(PathNode &node, const Grid &grid) {
  ...
  float dist_to_parent =
      glm::distance(grid.cells[node.cell_index]->transform.position,
